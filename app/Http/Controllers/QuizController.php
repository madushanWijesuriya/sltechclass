...
class QuizController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $path = $request->file('url')->move(public_path() . '/quiz_thumbnails/', $filename);

        try {

            $month = Month::find($request->input('month_id'));
            if ($month->quizzes()->create(collect($request->only(['name', 'description', 'embed_code']))->merge(['url' => $filename])->toArray()))
